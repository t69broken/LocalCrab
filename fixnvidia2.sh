...
if [ -f "$COMPOSE" ]; then
  ...
  if ! grep -q "NVIDIA_VISIBLE_DEVICES" "$COMPOSE"; then
    info "Adding NVIDIA_VISIBLE_DEVICES=all to ollama environment in docker-compose.yml…"
    # Insert after the last existing OLLAMA_ env var — no duplicate key risk
    python3 - "$COMPOSE" <<'PYEOF'
import sys, re
path = sys.argv[1]
text = open(path).read()
# Find the ollama environment block and append after the last OLLAMA_ line
inject = "      - NVIDIA_VISIBLE_DEVICES=all\n      - NVIDIA_DRIVER_CAPABILITIES=compute,utility\n"
# Only add if not already present
if "NVIDIA_VISIBLE_DEVICES" not in text:
    text = re.sub(
        r'((?:      - OLLAMA_[^\n]+\n)+)',
